...
describe("hooks utilities", () => {
  ...
  describe("useGetState", () => {
    ...
    it("should work with counter state", () => {
      const store = createStore(
        CounterState.Active({ value: 42, lastUpdated: "2024-01-01" }),
        CounterState
      );
      const context = createContext(store);
      const getState = useGetState<CounterStateType>()(context);
      ...
      expect(getState("Active", "lastUpdated")).toBe("2024-01-01");

      const activeProps = getState("Active")!;
      expect(activeProps.value).toBe(42);
      expect(activeProps.lastUpdated).toBe("2024-01-01");
    });
  ...
  describe("getStateProp", () => {
    ...
    it("should work with counter state", () => {
      const store = createStore(
        CounterState.Active({ value: 100, lastUpdated: "2024-12-31" }),
        CounterState
      );
      const context = createContext(store);
      const state = useStore(context);
      ...
      expect(activeProps.lastUpdated).toBe("2024-12-31");
    });
  });
